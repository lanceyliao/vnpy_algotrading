...
class BinanceLinearGateway(BaseGateway):
    ...
    def query_symbol_config(self, symbol: str = "") -> None:
        """query symbol config"""
        self.rest_api.get_symbol_config(symbol)
        ...
        if last_bar:
            ...
            if minutes_diff > 1:
                ...
                for i in range(1, minutes_diff):
                    last_bar.datetime = last_bar.datetime + timedelta(minutes=i)
...
class BinanceLinearRestApi(RestClient):
    ...
    def get_symbol_config(self, symbol: str = "") -> None:
        """获取交易对配置"""
        data: dict = {"security": Security.SIGNED}
        ...
        self.add_request(
            method="GET",
            path=path,
            callback=self.on_get_symbol_config,
            params=params,
            data=data
        )
    ...
    def on_get_symbol_config(self, data: list, request: Request) -> None:
        """获取交易对配置回调"""
        if data:
            gateway_name = self.gateway_name
            for config in data:
